...
class ContactController extends Controller
{
    ...
    public function send(): void
    {
        ...
        try {
            ...
            // DB yoksa ya da kolon eksikse yok say, mail yine de gitsin
        ...
        $eSubject = htmlspecialchars($subject ?: '—', ENT_QUOTES, 'UTF-8');
        $eMessage = nl2br(htmlspecialchars($message, ENT_QUOTES, 'UTF-8'));
        $eDate    = date('d.m.Y H:i');
        $eIp      = htmlspecialchars($_SERVER['REMOTE_ADDR'] ?? '', ENT_QUOTES, 'UTF-8');
        $emailRow = $email
            ? "<tr><td style=\"padding:10px 0;border-bottom:1px solid #f0f0f0;\"><strong style=\"color:#65758a;font-size:12px;text-transform:uppercase;\">E-posta</strong><br><a href=\"mailto:{$eEmail}\" style=\"color:#18C6C3;\">{$eEmail}</a></td></tr>"
            : '';
  ...
  <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,.08);">
    ...
    <tr><td style="padding:32px;">
      <table width="100%" cellpadding="0" cellspacing="0">
        <tr><td style="padding:10px 0;border-bottom:1px solid #f0f0f0;"><strong style="color:#65758a;font-size:12px;text-transform:uppercase;">Ad Soyad</strong><br><span style="color:#0A1F44;font-size:15px;font-weight:600;">{$eName}</span></td></tr>
        <tr><td style="padding:10px 0;border-bottom:1px solid #f0f0f0;"><strong style="color:#65758a;font-size:12px;text-transform:uppercase;">Telefon</strong><br><a href="tel:{$ePhone}" style="color:#18C6C3;font-size:15px;font-weight:600;">{$ePhone}</a></td></tr>
        {$emailRow}
        <tr><td style="padding:10px 0;border-bottom:1px solid #f0f0f0;"><strong style="color:#65758a;font-size:12px;text-transform:uppercase;">Konu</strong><br><span style="color:#0A1F44;font-size:15px;">{$eSubject}</span></td></tr>
        <tr><td style="padding:16px 0 0;"><strong style="color:#65758a;font-size:12px;text-transform:uppercase;">Mesaj</strong><br><p style="color:#4a5568;font-size:15px;line-height:1.7;margin:8px 0 0;">{$eMessage}</p></td></tr>
      </table>
    </td></tr>
    <tr><td style="background:#f8f9fb;padding:16px 32px;border-top:1px solid #eee;">
      <p style="margin:0;color:#aaa;font-size:12px;">Gönderim: {$eDate} | IP: {$eIp}</p>
    </td></tr>
  </table>
</td></tr></table>
</body></html>
HTML;
        ...
        $headers .= "From: Cakmaklar Insaat <noreply@cakmaklargrup.com>\r\n";
